...
/* Guest order rows are token-authorized behind the scenes. The customer
 * never needs to know about the token; it stays in localStorage and is sent
 * only to the security-definer RPC for the matching active order. */
    ...
    if (creds && url.includes('/rest/v1/delivery_orders') && !url.includes('/rpc/')) {
      ...
      if (orderId && orderId === creds.id && !/head=true/i.test(url)) {
        ...
        const response = await nativeFetch(rpcUrl, {
          ...
          method: 'POST',
          headers,
          body: JSON.stringify({ p_order_id: creds.id, p_token: creds.token }),
        });
        if (response.ok) {
...
export async function rpc(name, args = {}) {
  ...
  if (name === 'place_delivery_order' && result?.id && result?.trackingToken) {
    ...
    window.dispatchEvent(new CustomEvent('aquarium:order-tracking-ready', {
      detail: { id: result.id },
    }));
  }
  return result;
