...
/* ---------- drawer ---------- */
export function openCartExternal() { openCart(); }
...
export function initCart() {
  ...
  // Load an existing delivery order into the cart for the edit flow.
  document.addEventListener('delivery:edit-requested', (e) => {
    const items = Array.isArray(e.detail?.items) ? e.detail.items : [];
    cart = items.map((i) => ({
      id: Number(i.productId),
      name: i.name || '', nameAr: i.name_ar || '',
      categoryId: i.categoryId ?? null, price: +i.price || 0, image: i.image || '',
      qty: Math.min(MAX_QTY, Math.max(1, +i.quantity || 1)),
    })).filter((i) => i.id);
    persist(); renderCart(); renderBadge(); openCart();
  });
