...
import {
  ...
  getDeliveryOrder, getDeliveryHistory, subscribeDeliveryOrder, cancelDeliveryOrder, updateDeliveryOrder,
...
import { registerPushSubscription } from '../../shared/push.js';
...
export async function openDeliveryCheckout(cartSnapshot) {
  ...
  els.error.textContent = '';
  if (!editingOrder) removeCoupon();
  els.ptsInput.value = '';
  if (editingOrder) { coupon = null; els.coupon.value = ''; }
  ...
  paintSubZones();
  if (editingOrder) {
    zoneSel = zones.find((z) => Number(z.id) === Number(editingOrder.zoneId)) || null;
    subSel = zoneSel?.subZones?.find((z) => Number(z.id) === Number(editingOrder.subZoneId)) || null;
    subSkipped = !subSel;
    els.name.value = editingOrder.name || '';
    els.phone.value = editingOrder.phone || '';
    els.address.value = editingOrder.addressDetail || '';
    els.maps.value = editingOrder.mapsLink || '';
    els.notes.value = editingOrder.notes || '';
    const pay = els.payGroup.querySelector(`input[name="dlPay"][value="${CSS.escape(editingOrder.paymentMethod || 'cash')}"]`);
    if (pay && !pay.disabled) pay.checked = true;
    step = 3;
  }
  paintStep();
...
async function submit(e) {
  ...
  try {
    let order;
    if (editingOrder) {
      const updated = await updateDeliveryOrder({
        id: editingOrder.id, phone: v.phone, name: v.name, addressDetail: v.address,
        mapsLink: v.mapsLink, notes: v.notes, payment: v.payment,
        zoneId: String(zoneSel.id), subZoneId: subSel ? String(subSel.id) : '',
        userId: getUser()?.id || '',
        items: cartCache.items.map((i) => ({ productId: i.id, quantity: i.qty })),
      });
      order = await getDeliveryOrder(editingOrder.id);
    } else {
      const result = await placeDeliveryOrder({
        name: v.name, phone: v.phone, addressDetail: v.address, mapsLink: v.mapsLink,
        notes: v.notes, payment: v.payment, zoneId: String(zoneSel.id),
        subZoneId: subSel ? String(subSel.id) : '', userId: getUser()?.id || '',
        coupon: coupon?.code || '', redeemPoints: redeemPoints() || 0,
        items: cartCache.items.map((i) => ({ productId: i.id, quantity: i.qty })),
      });
      order = result.order;
    }
    ...
    if (!editingOrder) registerPushSubscription({ appRole: 'customer', userId: getUser()?.id || null, orderId: order.id }).catch(() => {});
    editingOrder = null;
...
function renderMeta(order) {
  ...
  if (els.actions) {
    const early = ['Received', 'Accepted'].includes(order.status);
    const canEdit = early && +order.discountAmount === 0 && +order.loyaltyRedeemed === 0 && !order.couponCode;
    els.actions.hidden = !early;
    if ($('tkEditBtn')) $('tkEditBtn').hidden = !canEdit;
    if ($('tkCancelBtn')) $('tkCancelBtn').hidden = !early;
  }
...
async function useCurrentLocationForMaps() {
  if (!navigator.geolocation) { toast(t('dl.mapNoGeo'), 'error'); return; }
  const btn = $('dlMapLocate');
  ...
  try {
    ...
    els.maps.value = `https://www.google.com/maps/search/?api=1&query=${latitude},${longitude}`;
    const open = $('dlMapOpen'); if (open) open.href = els.maps.value;
    toast(t('dl.mapSaved'));
  } catch (err) {
    toast(err?.code === 1 ? t('dl.mapDenied') : t('dl.mapFailed'), 'error');
  } finally { if (btn) btn.disabled = false; }
}

async function cancelTrackedOrder() {
  if (!trackedId || !['Received', 'Accepted'].includes((await getDeliveryOrder(trackedId)).status)) return;
  const order = await getDeliveryOrder(trackedId);
  const ok = window.confirm(t('tk.cancelConfirm'));
  if (!ok) return;
  try {
    await cancelDeliveryOrder(trackedId, order.phone || '');
    await loadTracking(trackedId);
    refreshPill(trackedId, 'Cancelled');
    toast(t('tk.cancelled'));
  } catch (err) { toast(err.message || t('auth.err.generic'), 'error'); }
}

async function editTrackedOrder() {
  if (!trackedId) return;
  try {
    const order = await getDeliveryOrder(trackedId);
    if (!['Received', 'Accepted'].includes(order.status)) return toast(t('tk.editTooLate'), 'error');
    if (+order.discountAmount > 0 || +order.loyaltyRedeemed > 0 || order.couponCode) return toast(t('tk.editDiscounted'), 'error');
    editingOrder = order;
    document.dispatchEvent(new CustomEvent('delivery:edit-requested', { detail: { items: order.items } }));
    closeLayer(els.track);
    setTimeout(() => openLayer(els.modal), 50);
    setTimeout(() => document.dispatchEvent(new CustomEvent('cart:editing-started')), 80);
  } catch (err) { toast(err.message || t('auth.err.generic'), 'error'); }
}
...
export function initDelivery() {
  ...
  $('dlMapLocate')?.addEventListener('click', useCurrentLocationForMaps);
  $('dlMapOpen')?.addEventListener('click', () => {
    const href = els.maps.value.trim() || 'https://www.google.com/maps';
    $('dlMapOpen').href = href;
  });
  $('tkCancelBtn')?.addEventListener('click', cancelTrackedOrder);
  $('tkEditBtn')?.addEventListener('click', editTrackedOrder);
