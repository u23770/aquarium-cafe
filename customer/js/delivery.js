...
import {
  ...
  getDeliveryOrder, getDeliveryHistory, subscribeDeliveryOrder, editDeliveryOrder, cancelDeliveryOrder, getProducts,
...
function renderMeta(order) {
  ...
  const editBtn = $('tkEditBtn');
  const cancelBtn = $('tkCancelBtn');
  const notifyBtn = $('tkNotifyBtn');
  if (editBtn) editBtn.hidden = !editable;
  if (cancelBtn) cancelBtn.hidden = !editable;
  if (notifyBtn) {
    notifyBtn.hidden = TERMINAL.includes(order.status);
...
function canEditOrder(order) {
  return ['Received', 'Accepted'].includes(order?.status) && !order?.loyaltyRedeemed && !order?.discountAmount;
}

function renderEditList(order) {
  const box = $('tkEditItems');
  if (!box) return;
  const items = Array.isArray(order?.items) ? order.items : [];
  const selected = new Map(items.map((it) => [Number(it.productId), Math.max(1, Number(it.quantity) || 1)]));
  box.innerHTML = `
    <div class="tk-edit-current">
      <h4>${esc(t('tk.currentItems'))}</h4>
      <div id="tkEditCurrent"></div>
    </div>
    <div class="tk-edit-add">
      <h4>${esc(t('tk.addItems'))}</h4>
      <input id="tkEditSearch" class="input" type="search" placeholder="${esc(t('tk.searchItems'))}" autocomplete="off">
      <div id="tkEditCatalog" class="tk-edit-catalog"></div>
    </div>`;
  renderEditCurrent(order, selected);
  renderEditCatalog(selected);

  $('tkEditSearch')?.addEventListener('input', () => renderEditCatalog(selected));
  box.dataset.selected = '1';
  box._editSelected = selected;
}

function renderEditCurrent(order, selected) {
  const box = $('tkEditCurrent');
  if (!box) return;
  const items = Array.isArray(order?.items) ? order.items : [];
  if (!items.length) {
    box.innerHTML = `<p class="tk-edit-empty">${esc(t('tk.editEmpty'))}</p>`;
    return;
  }
  box.innerHTML = items.map((it) => {
    const id = Number(it.productId);
    const q = selected.get(id) || 0;
    const name = document.documentElement.lang === 'ar' && it.name_ar ? it.name_ar : (it.name || '');
    return `<div class="tk-edit-item ${q === 0 ? 'is-removed' : ''}" data-edit-id="${id}">
      <div class="tk-edit-item__info"><b>${esc(name)}</b><small>${money(+it.price || 0)}</small></div>
      <div class="tk-edit-item__qty">
        <button type="button" data-edit-minus aria-label="minus">−</button>
        <strong data-edit-qty>${q}</strong>
        <button type="button" data-edit-plus aria-label="plus">+</button>
        <button type="button" data-edit-remove class="tk-edit-item__remove" aria-label="remove">×</button>
      </div>
    </div>`;
  }).join('');
}

async function renderEditCatalog(selected) {
  const box = $('tkEditCatalog');
  if (!box) return;
  try {
    const products = await getProducts('all');
    const q = ($('tkEditSearch')?.value || '').trim().toLowerCase();
    const list = (products || []).filter((p) => {
      if (!q) return true;
      return [p.name, p.name_ar, p.description, p.description_ar].filter(Boolean).some(v => String(v).toLowerCase().includes(q));
    }).slice(0, 60);
    box.innerHTML = list.map((p) => {
      const id = Number(p.id), qty = selected.get(id) || 0;
      const name = document.documentElement.lang === 'ar' && p.name_ar ? p.name_ar : p.name;
      return `<button type="button" class="tk-edit-product" data-edit-product="${id}">
        <span><b>${esc(name)}</b><small>${money(+p.price || 0)}</small></span>
        <strong>${qty ? `×${qty}` : '+'}</strong>
      </button>`;
    }).join('') || `<p class="tk-edit-empty">${esc(t('tk.noItems'))}</p>`;
  } catch (err) {
    box.innerHTML = `<p class="tk-edit-empty">${esc(err.message || t('tk.editFailed'))}</p>`;
  }
}

async function saveOrderEdit() {
  if (editBusy || !trackedId) return;
  const order = await getDeliveryOrder(trackedId);
  if (!canEditOrder(order)) {
    toast(t('tk.editUnavailable'), 'error');
    return;
  }
  const selected = $('tkEditItems')?._editSelected;
  const items = selected ? [...selected.entries()].filter(([,q]) => q > 0).map(([productId, quantity]) => ({ productId, quantity })) : [];
  if (!items.length) { toast(t('tk.editEmpty'), 'error'); return; }
  editBusy = true;
  const btn = $('tkSaveEdit');
  if (btn) btn.disabled = true;
  try {
    await editDeliveryOrder(trackedId, items);
    toast(t('tk.editSaved'));
    $('tkEditModal')?.classList.remove('open');
    await loadTracking(trackedId);
  } catch (err) {
    toast(err.message || t('tk.editFailed'), 'error');
  } finally {
    editBusy = false;
...
async function cancelCurrentOrder() {
  ...
  const order = await getDeliveryOrder(trackedId);
  if (!canEditOrder(order)) {
    toast(t('tk.cancelUnavailable'), 'error');
    return;
  }
  const ok = window.confirm(t('tk.cancelConfirm'));
  if (!ok) return;
  editBusy = true;
  const btn = $('tkCancelBtn');
  if (btn) btn.disabled = true;
  try {
    await cancelDeliveryOrder(trackedId);
    toast(t('tk.cancelled'));
    await loadTracking(trackedId);
  } catch (err) {
    toast(err.message || t('tk.cancelFailed'), 'error');
  } finally {
    editBusy = false;
    if (btn) btn.disabled = false;
  }
...
function openEditModal(order) {
  if (!canEditOrder(order)) { toast(t('tk.editUnavailable'), 'error'); return; }
  renderEditList(order);
  $('tkEditModal')?.classList.add('open');
}
...
export async function openTracking(id) {
  ...
  try {
    ...
    const editBtn = $('tkEditBtn');
    if (editBtn) editBtn.hidden = !canEditOrder(order);
    const notifyBtn = $('tkNotifyBtn');
    if (notifyBtn) notifyBtn.hidden = TERMINAL.includes(order.status);
    autoRegisterGrantedPush('customer', id).then(() => {
      if (Notification.permission === 'granted' && notifyBtn) notifyBtn.hidden = true;
    }).catch(() => {});
    startLive(id);
...
export function initDelivery() {
  ...
  $('tkEditBtn')?.addEventListener('click', async () => {
    if (!trackedId) return;
    try { openEditModal(await getDeliveryOrder(trackedId)); }
    catch (err) { toast(err.message, 'error'); }
  });
  $('tkSaveEdit')?.addEventListener('click', saveOrderEdit);
  $('tkEditCancel')?.addEventListener('click', () => $('tkEditModal')?.classList.remove('open'));
  $('tkCancelBtn')?.addEventListener('click', cancelCurrentOrder);
  $('tkNotifyBtn')?.addEventListener('click', async () => {
    if (!trackedId) return;
    const btn = $('tkNotifyBtn');
    try {
      btn.disabled = true;
      await registerPush('customer', trackedId);
      toast(t('tk.notificationsOn'));
      btn.hidden = true;
    } catch (err) {
      toast(err.message || t('tk.notificationsFailed'), 'error');
    } finally {
      btn.disabled = false;
    }
  });
  $('tkEditItems')?.addEventListener('click', (e) => {
    const host = $('tkEditItems');
    const selected = host?._editSelected;
    if (!selected) return;
    const row = e.target.closest('[data-edit-id]');
    const productBtn = e.target.closest('[data-edit-product]');
    if (productBtn) {
      const id = Number(productBtn.dataset.editProduct);
      const n = Math.min(20, (selected.get(id) || 0) + 1);
      selected.set(id, n);
      renderEditCatalog(selected);
      return;
    }
    ...
    let n = selected.get(id) || 0;
    if (e.target.closest('[data-edit-minus]')) n = Math.max(0, n - 1);
    if (e.target.closest('[data-edit-plus]')) n = Math.min(20, n + 1);
    if (e.target.closest('[data-edit-remove]')) n = 0;
    selected.set(id, n);
    const q = row.querySelector('[data-edit-qty]');
    if (q) q.textContent = String(n);
    row.classList.toggle('is-removed', n === 0);
    renderEditCatalog(selected);
  });

  /* Google Maps helper: use the phone's GPS and create a normal Google Maps pin URL.
     No Google API key is required. */
  $('dlPickMaps')?.addEventListener('click', () => {
    if (!navigator.geolocation) {
      els.error.textContent = t('val.mapsGeo');
      return;
    }
    const btn = $('dlPickMaps');
    const old = btn.innerHTML;
    btn.disabled = true;
    btn.textContent = t('dl.mapsLocating');
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const { latitude, longitude } = pos.coords;
        els.maps.value = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${latitude},${longitude}`)}`;
        btn.disabled = false;
        btn.innerHTML = old;
        toast(t('dl.mapsSaved'));
      },
      () => {
        btn.disabled = false;
        btn.innerHTML = old;
