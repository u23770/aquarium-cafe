...
--  20260807_v4_to_v5.sql before). Fresh installs already include
--  everything (schema.sql). The v4→v5 migration ships the same
--  fixes, so upgrading straight from v4 never needs this file.
--
--  WHAT THIS DOES (all IDEMPOTENT — safe to run more than once):
--    1. delivery_subzones gains  delivery_fee  (each sub zone carries
--       its OWN fee — it wins over the parent zone fee at checkout)
--       and  updated_at  (+ auto-touch trigger).
--    2. Existing sub zones are backfilled ONCE with their parent
--       zone's fee (behaviour exactly preserved; a marker row in
--       settings prevents re-runs from overwriting hand-tuned fees).
--    3. place_delivery_order() re-released: sub-zone fee override +
--       the  record "v_subzone" is not assigned yet  crash when a
--       customer skipped the sub-zone step (guest/zone-only orders).
--    4. _price_cart() re-released: fixes a record-field bug
--       (the function read v_product.name_ar without selecting
--       name_ar into the record → every order/coupon validation
--       raised  record "v_product" has no field "name_ar").
--
--  NOTHING destructive: no drops, no data changes to orders.
--  Historical orders keep their snapshotted delivery_fee untouched.
--
--  Run in: Supabase Dashboard → SQL Editor → New query → paste → Run.
-- ═══════════════════════════════════════════════════════════════════


-- ───────────────────── 1 · SUB-ZONE DELIVERY FEES ─────────────────
...
-- one-time backfill: pre-existing sub zones inherit the parent zone fee
do $$
begin
...
-- ───────────── 1b · website_content keys may be camelCase ─────────
-- navItems / footerAbout are the Visual Builder's native keys; the old
-- snake_case-only check rejected them (fresh seed + admin saves failed).
do $$
begin
  if exists (
    select 1 from pg_constraint
...
-- ──────────── 3 · place_delivery_order — sub-zone fee wins + ⚠ unassigned-record fix ────────────
...
-- ──────────── 4 · advance_delivery — return-object sync fix ───────────
-- the 'Delivered' step credited loyalty_earned to the row AFTER v_order
-- had been captured, so the RPC answered loyaltyEarned: 0 to the waiter
-- board. Re-released with the sync assignment.
...
-- grants unchanged (already granted in v5), re-stated for safety
...
-- pricing helpers stay INTERNAL: they are only ever called inside the
-- SECURITY DEFINER RPCs above (Postgres grants EXECUTE to PUBLIC by
-- default — revoke it so nobody can call the internals directly via RPC).
