...
export async function registerPush(role, orderId = null, existingRegistration = null) {
  ...
  // Use the explicitly registered Service Worker when available.
  // This avoids silently waiting forever if registration failed.
  const reg = existingRegistration || await navigator.serviceWorker.ready;
...
export async function autoRegisterGrantedPush(role, orderId = null, existingRegistration = null) {
  if ('Notification' in window && Notification.permission === 'granted') {
    try { return await registerPush(role, orderId, existingRegistration); } catch (err) {
